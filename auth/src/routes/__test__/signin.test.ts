...
it("fails when an email that does not exist is supplied", async () => {
  const response = await request(app).post("/api/users/signin").send({ email: "test@test.com", password: "password" }).expect(400);
...
it("fails when an incorrect password is supplied", async () => {
  ...
  const response = await request(app).post("/api/users/signin").send({ email: "test@test.com", password: "lkjahflkdsjh" }).expect(400);
  console.log(global.createMsg(expect.getState().currentTestName, "400", response));
});
